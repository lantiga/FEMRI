...
#
SET(LIBRARY_OUTPUT_PATH ${FEMRI_BINARY_DIR}/bin CACHE PATH
    "Single output directory for building all libraries.")
SET(EXECUTABLE_OUTPUT_PATH ${FEMRI_BINARY_DIR}/bin CACHE PATH
...
# Copy the CMake option to a setting with FEMRI_ prefix for use in
# our project.  This name is used in vtkbvgConfigure.h.in.
SET(FEMRI_BUILD_SHARED_LIBS ${BUILD_SHARED_LIBS})
...
IF (VTK_WRAP_TCL)
  ...
  OPTION(FEMRI_WRAP_TCL
         "Wrap classes into the TCL interpreted language."
         ON)

  IF(FEMRI_WRAP_TCL)
    SET(VTK_WRAP_TCL3_INIT_DIR "${FEMRI_SOURCE_DIR}/Wrapping")
    INCLUDE(${VTK_CMAKE_DIR}/vtkWrapTcl.cmake)
  ENDIF(FEMRI_WRAP_TCL)

ELSE (VTK_WRAP_TCL)

  IF (FEMRI_WRAP_TCL)
    MESSAGE("Warning. FEMRI_WRAP_TCL is ON but the VTK version you have "
            "chosen has not support for Tcl (VTK_WRAP_TCL is OFF).  "
            "Please set FEMRI_WRAP_TCL to OFF.")
    SET (FEMRI_WRAP_TCL OFF)
  ENDIF (FEMRI_WRAP_TCL)
...
ELSE (VTK_WRAP_PYTHON)
  ...
  IF (FEMRI_WRAP_PYTHON)
    MESSAGE("Warning. FEMRI_WRAP_PYTHON is ON but the VTK version you have "
            "chosen has not support for Python (VTK_WRAP_PYTHON is OFF).  "
            "Please set FEMRI_WRAP_PYTHON to OFF.")
    SET (FEMRI_WRAP_PYTHON OFF)
  ENDIF (FEMRI_WRAP_PYTHON)
...
IF (VTK_WRAP_JAVA)
  ...
  OPTION(FEMRI_WRAP_JAVA
         "Wrap classes into the Java interpreted language."
         ON)

  IF (FEMRI_WRAP_JAVA)
    SET(VTK_WRAP_JAVA3_INIT_DIR "${FEMRI_SOURCE_DIR}/Wrapping")
    ...
    SET(VTK_JAVA_HOME ${FEMRI_BINARY_DIR}/java/vtkbvg)
    MAKE_DIRECTORY(${VTK_JAVA_HOME})
  ENDIF (FEMRI_WRAP_JAVA)

ELSE (VTK_WRAP_JAVA)

  IF (FEMRI_WRAP_JAVA)
    MESSAGE("Warning. FEMRI_WRAP_JAVA is ON but the VTK version you have "
            "chosen has not support for Java (VTK_WRAP_JAVA is OFF).  "
            "Please set FEMRI_WRAP_JAVA to OFF.")
    SET (FEMRI_WRAP_JAVA OFF)
  ENDIF (FEMRI_WRAP_JAVA)

ENDIF (VTK_WRAP_JAVA)

# Setup our local hints file in case wrappers need them.
SET(VTK_WRAP_HINTS ${FEMRI_SOURCE_DIR}/Wrapping/hints)
